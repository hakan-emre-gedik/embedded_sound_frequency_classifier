...
;LABEL		DIRECTIVE	VALUE		COMMENT
			AREA    	UPDATE_SCREEN_EXTENDED, READONLY, CODE
			THUMB
			EXPORT  	UPDATE_SCREEN_EXTENDED; Make available
			IMPORT 		HIGH 
			IMPORT		LOW 
			IMPORT		LOW_FREQ_T
...
loopdots	LDR		R0, [R2], #1
			...
			LDR		R1, =AMPLITUDE_T
			LDR		R4, [R1]
			LDR		R5, =AMPLITUDE_DIGITS
			BL		CONVRT 
			POP	{R0-R12, LR}
			
			LDR		R1, =AMPLITUDE_DIGITS 
			PUSH {R0-R5, LR}
			BL		SCREEN_FIND_WHICH_NUMBERS
			POP	{R0-R5, LR}
